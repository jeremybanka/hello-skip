 ...
 */
export async function runService(service, options = {
    streaming_port: 8080,
    control_port: 8081,
}) {
    const runtime = await initService(service);
    const controlHttpServer = controlService(runtime).listen(options.control_port, () => {
        console.log(`Skip control service listening on port ${options.control_port.toString()}`);
    });
    const streamingHttpServer = streamingService(runtime).listen(options.streaming_port, () => {
        console.log(`Skip streaming service listening on port ${options.streaming_port.toString()}`);
    });
    return {
        close: () => {
